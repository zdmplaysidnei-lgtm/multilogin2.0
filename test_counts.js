...
async function test() {
  const { data: users } = await supabase.from('users').select('blocked');
  const blocked = users ? users.filter(u => u.blocked).length : 0;
  console.log(`Total users: ${users ? users.length : 0}, Blocked: ${blocked}`);
}
test();
