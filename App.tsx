...
const App: React.FC = () => {
   ...
   // --- GLOBAL DOWNLOADS ---
   const [globalDownloads, setGlobalDownloads] = useState<{ id: string; fileName: string; progress: number; state: 'downloading' | 'success' | 'error' }[]>([]);

   // 📥 Registra listener global de downloads (navegador interno e externo)
   useEffect(() => {
      if (!window.nebulaAPI?.onDownloadProgress) return;
      window.nebulaAPI.onDownloadProgress((data: any) => {
         if (data.profileId === 'global') {
            ...
            setGlobalDownloads(prev => {
               ...
               const updated = {
                  ...
                  state: data.state as 'downloading' | 'success' | 'error'
               };
               if (exists) return prev.map(d => d.id === id ? updated : d);
               return [...prev, updated];
            });
            if (data.state !== 'downloading') {
               setTimeout(() => setGlobalDownloads(prev => prev.filter(d => d.id !== id)), 6000);
            }
         }
      });
   }, []);
                     ...
                     <>
                        ...
                        <div className="bg-[#111] border border-gray-800 rounded-[40px] p-10 space-y-8 shadow-2xl border-l-purple-500 border-l-8">
                           ...
                           <div className="space-y-2 pb-4">
                              <label className="text-[10px] font-black uppercase text-gray-500">Público-Alvo</label>
                              <div className="grid grid-cols-3 gap-2 bg-black/40 p-1 rounded-xl border border-gray-800">
                                 {[
                                    { id: 'all', label: '👥 Todos' },
                                    { id: 'admin', label: '👑 Admins' },
                                    { id: 'resellers', label: '🏪 Revendedores' },
                                    { id: 'members', label: '👤 Todos Membros' },
                           ...
                           <div className="bg-[#111] border border-gray-800 rounded-[40px] p-10 space-y-8 shadow-2xl">
                              ...
                              <div className="space-y-6">
                                 ...
                                 <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                                       {[
                                          { id: 'nebula', label: '🌌 Nebula' },
                                          { id: 'fireworks', label: '🎆 Ano Novo' },
                                          { id: 'carnival', label: '🎭 Carnaval' },
                                          { id: 'stpatricks', label: '🍀 St Patrick' },
                                          { id: 'easter', label: '🐰 Páscoa' },
                                          { id: 'mothersday', label: '💐 Dia das Mães' },
                                          { id: 'junina', label: '🏮 Festa Junina' },
                                          { id: 'ocean', label: '🌊 Oceano' },
                                          { id: 'aurora', label: '🌈 Aurora' },
                                          { id: 'spring', label: '🌻 Primavera' },
                                          { id: 'halloween', label: '🎃 Halloween' },
                                          { id: 'leaves', label: '🍂 Outono' },
                           ...
                           <div className="grid grid-cols-1 gap-6 relative z-10">
                              <div className="bg-blue-600/5 border border-blue-500/20 p-6 rounded-2xl space-y-4">
                                 ...
                                 <div className="flex items-center gap-3 bg-black/40 p-4 rounded-xl border border-white/5">
                                    <code className="flex-1 text-[11px] text-blue-200 font-mono break-all select-all">https://fkrijejmvtwwtgirlsey.supabase.co/functions/v1/smart-action</code>
                                    <button onClick={() => { navigator.clipboard.writeText('https://fkrijejmvtwwtgirlsey.supabase.co/functions/v1/smart-action'); setToast({ msg: 'Copiado!', type: 'info' }) }} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 transition-all"><Copy size={16} /></button>
                                 </div>
                              </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-8">
               <div className="grid grid-cols-2 gap-10">
                  <div className="space-y-6">
                     ...
                     {/* STATUS DO PERFIL: ATIVO / MANUTENÇÃO */}
                     <div className="space-y-1">
                        <label className="text-[10px] font-black uppercase text-gray-600">Status do Perfil</label>
                        <div className="flex gap-3">
                           <label className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border-2 cursor-pointer transition-all text-sm font-bold uppercase tracking-wider ${(!editingProfile || editingProfile.status === 'active') ? 'bg-green-600/20 border-green-500 text-green-400 shadow-[0_0_15px_rgba(34,197,94,0.2)]' : 'bg-[#111] border-gray-800 text-gray-600 hover:border-gray-600'
                              }`}>
                              <input type="radio" name="status" value="active" defaultChecked={!editingProfile || editingProfile.status === 'active'} className="hidden" />
                              <span className="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></span>
                              ATIVO
                           </label>
                           <label className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border-2 cursor-pointer transition-all text-sm font-bold uppercase tracking-wider ${editingProfile?.status === 'maintenance' ? 'bg-red-600/20 border-red-500 text-red-400 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : 'bg-[#111] border-gray-800 text-gray-600 hover:border-gray-600'
                              }`}>
                              <input type="radio" name="status" value="maintenance" defaultChecked={editingProfile?.status === 'maintenance'} className="hidden" />
                     ...
                     <div className="flex items-center gap-4 bg-orange-900/10 border border-orange-500/30 rounded-2xl p-4">
                        <input type="checkbox" name="useExternalBrowserUI" id="useExternalBrowserUI" defaultChecked={editingProfile?.useExternalBrowserUI} className="w-5 h-5 accent-orange-500" />
                        <div className="flex flex-col">
                           <label htmlFor="useExternalBrowserUI" className="text-sm font-bold text-orange-400 cursor-pointer">🌐 Navegação Normal</label>
                           <span className="text-[9px] text-gray-500">Abre navegador completo (sem Modo App) permitindo instalar extensões</span>
                        </div>
                     </div>
                     ...
                     <div className="flex items-center gap-4 bg-blue-900/10 border border-blue-500/30 rounded-2xl p-4">
                        <input type="checkbox" name="enableExtensions" id="enableExtensions" defaultChecked={editingProfile?.enableExtensions} className="w-5 h-5 accent-blue-500" />
                        <div className="flex flex-col">
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(err.message, { status: 500 })
                  ...
                  <button onClick={() => {
                     navigator.clipboard.writeText(`import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'
...
})`); setToast({ msg: 'Copiado!', type: 'success' });
                  }} className="absolute top-4 right-4 p-3 bg-purple-600 rounded-xl opacity-0 group-hover:opacity-100 transition-all cursor-pointer"><Copy size={16} /></button>
               </div>
            </div>
         </Modal>
         ...
         <Modal isOpen={showImportModal} onClose={() => setShowImportModal(false)} title="Importador em Massa (Auto 30 Dias)">
            <form onSubmit={async e => {
               ...
               try {
                  ...
                  const deltaToUpsert: typeof users = [];
                  const nextUsers = [...users];

                  emails.forEach(email => {
                     let existing = nextUsers.find(u => u?.email?.toLowerCase() === email);
                     if (existing) {
                        existing.expirationDate = thirtyDaysFromNow;
                        existing.blocked = false;
                        updatedCount++;
                        deltaToUpsert.push(existing);
                     } else {
                        const newUser = {
                           id: `u_imp_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`, email, password: settings?.defaultMemberPassword || 'membro123', role: Role.MEMBER, ownerId: currentUser?.id || 'ADMIN', createdAt: Date.now(), expirationDate: thirtyDaysFromNow, blocked: false, isLoggedIn: false
                        };
                        nextUsers.push(newUser);
                        addedCount++;
                        deltaToUpsert.push(newUser);
                     }
                  });

                  if (deltaToUpsert.length === 0) {
                     setToast({ msg: 'Nenhum e-mail válido para processar.', type: 'info' });
                     setIsSaving(false); return;
                  }

                  setUsers(nextUsers);
                  setToast({ msg: `Salvando ${deltaToUpsert.length} alterações na Nuvem...`, type: 'info' });
                  ...
                  localStorage.setItem('nebula_users_v1', Security.encrypt(nextUsers));
                  setShowImportModal(false); setImportEmails('');

                  if (allSuccess) {
                     setToast({ msg: `✅ Cloud Sync OK! ${addedCount} novos, ${updatedCount} renovados.`, type: 'success' });
                  } else {
                     setToast({ msg: `⚠️ Importado com inconsistências na Nuvem. Verifique a tela.`, type: 'error' });
                  }
               } catch (err) {
                  console.error("Crash no Importador:", err);
                  setToast({ msg: 'Erro Crítico: ' + (err as Error).message, type: 'error' });
               } finally { setIsSaving(false); }
            }} className="space-y-6">
         ...
         {launchingStatus.isLaunching && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in">
               <div className="flex flex-col items-center gap-6 p-10 bg-gradient-to-br from-gray-900/90 to-black/90 rounded-3xl border border-red-500/30 shadow-2xl shadow-red-500/20">
                  {/* Spinner animado */}
                  <div className="relative">
                     <div className="w-16 h-16 border-4 border-purple-500/30 rounded-full animate-spin" style={{ borderTopColor: '#E50914' }}></div>
         ...
         {globalDownloads.length > 0 && (
            <div className="fixed bottom-5 right-5 z-[9999] flex flex-col gap-2 pointer-events-none">
               {globalDownloads.map(dl => (
                  <div key={dl.id} className="flex items-center gap-3 bg-[#1a1a1a]/95 border border-gray-700 rounded-xl px-4 py-3 shadow-2xl backdrop-blur-md pointer-events-auto min-w-[280px] max-w-xs animate-fade-in-up">
                     <div className="shrink-0">
                        {dl.state === 'downloading' && (
                           <div className="p-1.5 bg-purple-600/20 rounded-lg">
                              <DownloadCloud className="w-4 h-4 text-purple-400 animate-bounce" />
                           </div>
                        )}
                        {dl.state === 'success' && (
                     ...
                     <div className="flex-1 min-w-0">
                        <p className="text-white text-[10px] font-black truncate tracking-wide uppercase">{dl.fileName}</p>
                        <div className="flex items-center gap-2 mt-1">
                           <div className="flex-1 h-1 bg-gray-800 rounded-full overflow-hidden">
                              <div
                                 className={`h-full rounded-full transition-all duration-300 ${dl.state === 'success' ? 'bg-green-500' : dl.state === 'error' ? 'bg-red-500' : 'bg-gradient-to-r from-purple-500 to-pink-500'
                                    }`}
                                 style={{ width: `${dl.progress}%` }}
                              />
                           </div>
                           <span className="text-gray-500 text-[9px] font-bold shrink-0">
                              {dl.state === 'downloading' ? `${dl.progress}%` : dl.state.toUpperCase()}
                           </span>
                        </div>
                     </div>
                     <button
                        onClick={() => setGlobalDownloads(prev => prev.filter(d => d.id !== dl.id))}
