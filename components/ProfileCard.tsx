...
export const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`group flex flex-col bg-[#1a1a1a] rounded-xl overflow-hidden border border-gray-800 hover:border-[#9333EA] transition-all duration-300 hover:shadow-[0_0_30px_rgba(147,51,234,0.15)] h-[340px] relative
        ${isDragging ? 'opacity-20 scale-95 border-dashed border-[#9333EA]' : 'opacity-100'} 
    ...
    >
      ...
      <div className="flex-1 flex flex-col p-4 bg-[#1a1a1a] relative">
        ...
        {/* Título */}
        <div className="mb-3">
          <h3 className="font-bold text-base text-gray-100 truncate group-hover:text-[#D8B4FE] transition-colors" title={profile.name}>
            {profile.name}
          </h3>
        ...
        <div className="flex items-center justify-between mb-4">
          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded border flex items-center gap-1 ${isMaintenance ? 'bg-[#9333EA]/20 text-[#D8B4FE] border-purple-900/30' : 'bg-green-900/20 text-green-400 border-green-500/30'}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${isMaintenance ? 'bg-[#9333EA]' : 'bg-green-500 animate-pulse'}`}></span>
        ...
        <button
          ...
          className={`relative w-full py-3 rounded-full font-bold text-sm tracking-wider flex items-center justify-center gap-2 transition-all duration-300 transform active:scale-95 group/btn mb-3 overflow-hidden
            ${isMaintenance
              ...
              : 'bg-gradient-to-r from-[#9333EA] via-[#9333EA] to-[#7E22CE] text-white shadow-[0_0_25px_rgba(147,51,234,0.5)] hover:shadow-[0_0_40px_rgba(147,51,234,0.7)] hover:scale-[1.03] hover:brightness-110'
            }`}
        >
          ...
          <div className="flex items-center gap-2 pt-3 border-t border-gray-800/50 mt-auto">
            ...
            {onDelete && (
              <button
                ...
                className="px-3 py-1.5 rounded bg-red-900/10 hover:bg-red-900/30 text-[#9333EA] hover:text-[#D8B4FE] transition-colors border border-red-900/20"
              >
