...
async function test() {
  const { data: users } = await supabase.from('users').select('email, blocked, expirationDate').limit(5);
  console.log('Users DB:', users);
  const { data: profiles } = await supabase.from('profiles').select('name, status').limit(2);
